 ...
 * @// TODO: 2020/9/18
 */
public class VRCameraView extends GLSurfaceView implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {

    private VRCameraDrawer mVRCameraDrawer;
    ...
    private void init() {
        ...
        /**初始化Camera的绘制类*/
        mVRCameraDrawer = new VRCameraDrawer(getResources());
        /**初始化相机的管理类*/
    ...
    public void switchCamera() {
        ...
        mVRCameraDrawer.switchCamera();
        open(cameraId);
    }

    @Override
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        mVRCameraDrawer.onSurfaceCreated(gl, config);
        if (!isSetParm) {
            open(cameraId);
            stickerInit();
        }
        mVRCameraDrawer.setPreviewSize(dataWidth, dataHeight);
    }

    @Override
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        mVRCameraDrawer.onSurfaceChanged(gl, width, height);
    }

    @Override
    public void onDrawFrame(GL10 gl) {
        if (isSetParm) {
            mVRCameraDrawer.onDrawFrame(gl);
        }
    }
    ...
    public VRBeautifyLevel getBeautyLevel() {
        return mVRCameraDrawer.getBeautyLevel();
    }

    public void changeBeautyLevel(final VRBeautifyLevel level) {
        queueEvent(new Runnable() {
            @Override
            public void run() {
                mVRCameraDrawer.changeBeautyLevel(level);
            }
        });
    }
    ...
    public void stopRecord() {
        queueEvent(new Runnable() {
            @Override
            public void run() {
                mVRCameraDrawer.stopRecord();
            }
        });
    }

    public void setSavePath(String path) {
        mVRCameraDrawer.setSavePath(path);
    }
