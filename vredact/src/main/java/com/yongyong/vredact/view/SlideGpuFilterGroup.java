...
public class SlideGpuFilterGroup {
    ...
    public SlideGpuFilterGroup() {
        initFilter();
        scroller = new Scroller(VRKit.getAppContext());
    }
    ...
    public void onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                locked = true;
                downX = -1;
                if (offset > VRScreenTool.getScreenWidth() / 3) {
                    scroller.startScroll(offset, 0, VRScreenTool.getScreenWidth() - offset, 0, 100 * (1 - offset / VRScreenTool.getScreenWidth()));
                    needSwitch = true;
                } else {
                    scroller.startScroll(offset, 0, -offset, 0, 100 * (offset / VRScreenTool.getScreenWidth()));
                    needSwitch = false;
                }
                break;
        }
